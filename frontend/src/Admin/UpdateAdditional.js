...
const UpdateAdditional = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:8000/getadditional/${id}`)
    .then((res)=>{
        const data = res.data;
        setFormData({
          name :data.name,
          ...
          price:data.price,
          boardingAndDaycare:data.boardingAndDaycare,
  ...
  const handleSubmit = (e) => {
    ...
    axios.put(`http://localhost:8000/updateadditional/${id}`, {
      name,
      description,
      price,
      boardingAndDaycare,
  ...
  return (
    ...
    <div className="flex justify-center">
      <div className="w-96 border border-gray-300 p-4 rounded-md bg-gray-100" style={{width:"550px"}}>
        <h2 className='text-center text-2xl'>Update Additional Service</h2>
        <form onSubmit={handleSubmit}>
          ...
          <label >Includes Boarding And Daycare:</label>
            <input type="checkbox" name="boardingAndDaycare" checked={formData.boardingAndDaycare} onChange={handleChange} style={{marginLeft:"55px"}}/>
          </div>
            <div>
            <label >Includes Training Classes:</label>
            <input type="checkbox" name="trainingClasses" checked={formData.trainingClasses} onChange={handleChange}  style={{marginLeft:"55px"}} />
         </div>  
